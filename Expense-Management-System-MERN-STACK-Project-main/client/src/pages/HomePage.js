...
const HomePage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      message.error(error.response?.data?.message || "Failed to save asset. Please try again.");
    }
  };
              ...
              <Col xs={24} lg={12}>
                ...
                >
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie
                        data={Object.entries(analytics.assetTypeDistribution).map(([name, value]) => ({ name, value }))}
                        cx="50%"
                        cy="50%"
                        labelLine={false}
                        ...
                        dataKey="value"
                      >
                        {Object.entries(analytics.assetTypeDistribution).map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={[THEME.primary, THEME.success, 'var(--success)', '#90EE90', '#98FB98'][index % 5]} />
                        ))}
                      </Pie>
                      <RechartsTooltip />
                    </PieChart>
